...
export async function POST(request: Request) {
  ...
  const { exp, ...userInfoWithoutExp } = userInfo as {
    exp: number;
    sub: string;
    name: string;
    email: string;
  ...
  const refreshToken = await new jose.SignJWT({
    ...
    name: (userInfo as { name: string }).name,
    email: (userInfo as { email: string }).email,
    picture: (userInfo as { picture: string }).picture,
    given_name: (userInfo as { given_name: string }).given_name,
    family_name: (userInfo as { family_name: string }).family_name,
    email_verified: (userInfo as { email_verified: boolean }).email_verified,
  })
    .setProtectedHeader({ alg: 'HS256' })
